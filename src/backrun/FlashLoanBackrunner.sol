...
contract FlashLoanBackrunner is IFlashLoanSimpleReceiver, Ownable, ReentrancyGuard {
    ...
    event BackrunOpportunityDetected(
        PoolId indexed poolId, uint256 targetPrice, uint256 currentPrice, uint256 backrunAmount, bool zeroForOne
    );

    event BackrunExecuted(
        PoolId indexed poolId, uint256 flashLoanAmount, uint256 profit, uint256 lpShare, address keeper
    );
    ...
    constructor(IPoolManager _poolManager, address _aavePool) Ownable(msg.sender) {
        poolManager = _poolManager;
        if (_aavePool != address(0)) {
            aavePool = IAavePool(_aavePool);
        } else if (block.chainid == 11155111) {
            aavePool = IAavePool(AAVE_POOL_SEPOLIA);
        } else if (block.chainid == 1) {
            aavePool = IAavePool(AAVE_POOL_MAINNET);
        } else {
            // Unknown chain: leave unset. Flash-loan execution will fail unless explicitly configured.
    ...
    function executeBackrun(PoolId poolId, uint256 minProfit) external onlyKeeper nonReentrant {
    ...
    function executeBackrunPartial(PoolId poolId, uint256 flashLoanAmount, uint256 minProfit)
        external
        onlyKeeper
        nonReentrant
    {
    ...
    function executeBackrunWithCapital(PoolId poolId, uint256 amountIn, uint256 minProfit)
        external
        onlyKeeper
        nonReentrant
    {
        ...
        address tokenIn =
            opp.zeroForOne ? Currency.unwrap(opp.poolKey.currency0) : Currency.unwrap(opp.poolKey.currency1);
    ...
    function executeBackrunFor(PoolId poolId, uint256 minProfit, address keeper) external onlyForwarder nonReentrant {
    ...
    /// @notice Execute a pending backrun using a flash loan on behalf of an authorized keeper, but for a smaller amount.
    /// @dev This is the forwarder-friendly counterpart of `executeBackrunPartial`.
    ...
    {
        if (!authorizedKeepers[keeper]) revert UnauthorizedKeeper();
        _executeBackrun(poolId, flashLoanAmount, minProfit, keeper);
    }

    function _executeBackrun(PoolId poolId, uint256 flashLoanAmount, uint256 minProfit, address profitRecipient)
        internal
    {
        BackrunOpportunity storage opp = pendingBackruns[poolId];
        ...
        if (!repayPoolKeySet[poolId]) revert RepayPoolNotSet(poolId);
        if (address(aavePool) == address(0)) revert FlashLoanFailed();
        ...
        returns (bool)
    {
        ...
        uint256 amountOut = _executeBackrunSwap(flashParams.poolKey, flashParams.zeroForOne, amount);

        // Calculate profit
        uint256 totalOwed = amount + premium;

        // Swap on an external venue/pool to get back the borrowed asset for repayment.
        uint256 amountBack = _executeReverseSwap(flashParams.repayPoolKey, !flashParams.zeroForOne, amountOut);
    ...
    function checkProfitability(PoolId poolId) external view returns (bool profitable, uint256 estimatedProfit) {
        BackrunOpportunity storage opp = pendingBackruns[poolId];
    ...
    function _executeBackrunSwap(PoolKey memory key, bool zeroForOne, uint256 amountIn)
        internal
    ...
    function _executeReverseSwap(PoolKey memory key, bool zeroForOne, uint256 amountIn)
        internal
        returns (uint256 amountOut)
    {
        address tokenIn = zeroForOne ? Currency.unwrap(key.currency0) : Currency.unwrap(key.currency1);
    ...
    function _distributeProfit(PoolKey memory key, address profitToken, uint256 profit, address keeper) internal {
    ...
    function getPendingBackrun(PoolId poolId)
        external
        view
        returns (
            uint256 targetPrice,
