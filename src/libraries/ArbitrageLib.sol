...
library ArbitrageLib {
    ...
    function calculatePriceBounds(uint256 oraclePrice, uint256 oracleConfidence)
        internal
        pure
    ...
    function isOutsideConfidenceBand(uint256 poolPrice, uint256 oraclePrice, uint256 oracleConfidence)
        internal
        pure
        returns (bool isOutside)
    {
    ...
    function calculateDivergenceBps(uint256 poolPrice, uint256 oraclePrice)
        internal
        pure
    ...
    function isArbitrageAdvantageous(uint256 poolPrice, uint256 oraclePrice, bool zeroForOne)
        internal
        pure
        returns (bool isAdvantageous)
    {
        if (zeroForOne) {
    ...
    function calculateArbitrageOpportunity(ArbitrageParams memory params) internal pure returns (uint256 opportunity) {
        if (params.swapAmount == 0 || params.oraclePrice == 0 || params.poolPrice == 0) {
            return 0;
        }

        // Calculate bounds with confidence
        (uint256 priceLower, uint256 priceUpper) = calculatePriceBounds(params.oraclePrice, params.oracleConfidence);
    ...
    function calculateHookShare(uint256 arbitrageOpp, uint256 hookShareBps) internal pure returns (uint256 hookShare) {
    ...
    function analyzeArbitrageOpportunity(ArbitrageParams memory params, uint256 hookShareBps, uint256 minDivergenceBps)
        internal
        pure
        returns (ArbitrageResult memory result)
    {
        ...
        result.isOutsideConfidenceBand =
            isOutsideConfidenceBand(params.poolPrice, params.oraclePrice, params.oracleConfidence);

        // Determine if we should interfere
        // Must be: outside confidence band AND divergence > threshold AND advantageous direction
        result.shouldInterfere = result.isOutsideConfidenceBand && result.priceDivergenceBps >= minDivergenceBps
    ...
    function calculateBackrunAmount(uint256 poolPrice, uint256 oraclePrice, uint128 liquidity, bool zeroForOne)
        internal
        pure
